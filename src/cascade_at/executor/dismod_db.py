...
def main(argv):
    ...
    args = ARG_LIST.parse_args(argv[1:])
...
if __name__ == '__main__':
    import shutil
    sys.argv = 'dismod_db --model-version-id 475588 --parent-location-id 523 --sex-id 3 --fill --test-dir /tmp'.split()
    dismod_cmds = []
    if '--dm-commands' in sys.argv:
        dismod_cmds = sys.argv[sys.argv.index('--dm-commands')+1:]
    if 'ODE' in dismod_cmds:
        print ("The command 'ODE fit' is implemented in a wrapper for dmdismod.")
    print (sys.argv)
    main(sys.argv)
